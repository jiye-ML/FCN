...
class FCN_VGGNet:
    ...
    def vgg_19(self, input_op, **kw):
        ...
        with tf.variable_scope("inference"):
            ...
            # [8, 8]
            pool5 = tf.nn.max_pool(conv_final_layer, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME") # 7x7x512
            ...
            conv6 = tf.nn.bias_add(tf.nn.conv2d(pool5, W6, strides=[1, 1, 1, 1], padding="SAME"), b6) # 7x7x4096
            ...
            conv8 = tf.nn.bias_add(tf.nn.conv2d(relu_dropout7, W8, strides=[1, 1, 1, 1], padding="SAME"), b8)  # 7x7x151

            # now to upscale to actual image size
            deconv_shape1 = vgg_net["pool4"].get_shape() # 14x14x512
            ...
            conv_t1 = tf.nn.bias_add(tf.nn.conv2d_transpose(conv8, W_t1, tf.shape(vgg_net["pool4"]), strides=[1, 2, 2, 1], padding="SAME"), b_t1) # 14x14x512
            fuse_1 = tf.add(conv_t1, vgg_net["pool4"], name="fuse_1") # 14x14x512

            deconv_shape2 = vgg_net["pool3"].get_shape() # 28x28x256
            ...
            conv_t2 = tf.nn.bias_add(tf.nn.conv2d_transpose(fuse_1, W_t2, tf.shape(vgg_net["pool3"]), strides=[1, 2, 2, 1], padding="SAME"), b_t2) # 28x28x256
            fuse_2 = tf.add(conv_t2, vgg_net["pool3"], name="fuse_2") # 28x28x256
